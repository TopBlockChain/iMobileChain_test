...
4、矿工通过访问能量加注站，由能量加注站验证后使用Refuel函数对该用户进行能量加注；
5、在能量有效期1小时内，矿工使用Mine函数挖矿,若有推荐人将为其推荐人奖励10%，为推荐人的上级机构奖励5%(若有），为矿池奖励1%，其余奖励给矿工；
*/
//引用燃料加注合约中的变量
...
//推荐函数，记录下推荐人
  function Refering(address _Referer) public {
        require(Referees[msg.sender]==0x0);  //需要新矿工未被推荐过，其推荐人参数为0；
        Referees[msg.sender]=_Referer;      //设置新矿工的推荐人
    }
